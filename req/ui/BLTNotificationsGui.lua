...
function BLTNotificationsGui:_update_bars()
	...
	local db_entry_exist = (blt and blt.db_create_entry) or (DB and DB.create_entry)
	for i = 1, self._notifications_count do
		local page_button = self._buttons_panel:bitmap(db_entry_exist and {
			name = tostring(i),
...
function BLTNotificationsGui:update(t, dt)
	...
	if pending_downloads_count > 0 then
		...
		self._downloads_count:set_text(managers.experience:experience_string(pending_downloads_count))
	else
		self._downloads_panel:set_visible(false)
	end
